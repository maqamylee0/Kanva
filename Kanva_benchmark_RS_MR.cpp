#include <iostream>
#include "functions.h"
#include "Kanva_RS/kanva_RS.h"
#include "Kanva_RS/kanva_RS_impl.h"

typedef kanva_RS::Kanva_RS<key_type, val_type> DS;
...
int main(int argc, char **argv) {
    ...
    load_data();
    kanva_RS::Kanva_RS<key_type, val_type>* ds;
    prepare(ds);
...
void prepare(DS *&ds){
    ...
    TIMER_BEGIN(0);
    size_t maxErr = 4;
    ds = new kanva_RS::Kanva_RS<key_type, val_type>(Config.thread_num);
    ds->train(exist_keys, exist_keys, 32, 0);
...
void *run_fg(void *param) {
    ...
    while (running) {
        ...
        if (d <= Config.read_ratio) {                   // search
            ...
            ds->find(dummy_key, dummy_value, thread_id);
            query_i++;
            ...
            ds->insert(dummy_key, dummy_key, thread_id);
            insert_i++;
            if (unlikely(insert_i == data1.size())) {
                insert_i = 0;
            }
        } else if (d <= Config.read_ratio+Config.insert_ratio + Config.delete_ratio){               // remove
            key_type dummy_key = (exist_keys)[delete_i % exist_keys.size()];
            ds->remove(dummy_key, thread_id);
            delete_i++;
            ...
            key_type dummy_key = (exist_keys)[delete_i % exist_keys.size()];
            std::vector<std::pair<key_type , key_type>> result;
            ds->scan(dummy_key, 1000, result, thread_id);
            delete_i++;
            if (unlikely(delete_i == exist_keys.size())) {
                delete_i = 0;
            }
