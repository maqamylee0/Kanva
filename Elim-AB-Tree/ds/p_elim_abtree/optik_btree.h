...
#include <x86intrin.h> // clflush

/* Persistent memory things */
// https://stackoverflow.com/a/66393118
template<typename S>
inline void flushStruct(S* s) {
    const int LINESIZE = 64;
    char *p = (char *)s;
    uintptr_t endline = ((uintptr_t)s + sizeof(S) - 1) | (LINESIZE-1);
...
class Node {
  public:
    ...
    int size;
    K keys[DEGREE];
    Node<K,V,DEGREE>* volatile ptrs[DEGREE]; // also doubles as a spot for VALUES
...
class ABTreeOptik {
  ...
  private:
    bool dirty(nodeptr p);
    void flushPtr(nodeptr volatile * const pAddr);
    nodeptr markDirty(nodeptr p);
    nodeptr readUntilFlushed(nodeptr volatile * const addr);
...
/* NVRAM helpers */
template <class RecordManager, typename K, typename V, int DEGREE, class Compare>
inline bool ABTreeOptik<RecordManager, K, V, DEGREE, Compare>::dirty(nodeptr p) {
    return ((uintptr_t)p) & 1;
}

template <class RecordManager, typename K, typename V, int DEGREE, class Compare>
inline void ABTreeOptik<RecordManager, K, V, DEGREE, Compare>::flushPtr(nodeptr volatile * const pAddr) {
    // node containing p should be locked
    assert(dirty(*pAddr));
    *pAddr = (nodeptr)(((uintptr_t)(*pAddr)) - 1);
    assert(!dirty(*pAddr));
    // Have to cast away volatile
    flush((void*)pAddr);
}

template <class RecordManager, typename K, typename V, int DEGREE, class Compare>
inline nodeptr ABTreeOptik<RecordManager, K, V, DEGREE, Compare>::markDirty(nodeptr p) {
    assert(!dirty(p));
    return (nodeptr)(((uintptr_t)p) | 1);
}

template <class RecordManager, typename K, typename V, int DEGREE, class Compare>
inline nodeptr ABTreeOptik<RecordManager, K, V, DEGREE, Compare>::readUntilFlushed(nodeptr volatile * const pAddr) {
    nodeptr p = *pAddr;
    while (dirty(p)) {
        _mm_pause();
        p = *pAddr;
...
V ABTreeOptik<RecordManager, K, V, DEGREE, Compare>::searchBasic(const int tid, const K &key) {
    nodeptr node = readUntilFlushed(&entry->ptrs[0]);
    while (!node->leaf) {
        int idx = getChildIndex(node, key);
        node = readUntilFlushed(&node->ptrs[idx]);
    }
    V retval;
...
int ABTreeOptik<RecordManager, K, V, DEGREE, Compare>::trySearch(const int tid, SearchInfo &info, const K &key, nodeptr target) {
    ...
    info.oParent = entry;
    info.nodeIndex = 0;
    info.oNode = readUntilFlushed(&entry->ptrs[0]);
    while (!info.oNode->leaf && (target ? info.oNode != target : true)) {
        ...
        info.oNode = readUntilFlushed(&info.oNode->ptrs[info.nodeIndex]);
    }
    if (target) {
    ...
    if (currSize < b) {
        ...
        for (int i = 0; i < DEGREE; ++i) {
            if (node->keys[i] == (K) 0) {
                ...
                // Flush and sync value first so that the insert happens atomically in NVRAM
                // at the write of key
                node->ptrs[i] = (nodeptr) value;
                flush((void*)&node->ptrs[i]); // have to cast away the volatile
                sync();
                node->keys[i] = key;
                ++node->size;
                flush(&node->keys[i]);
        ...
        flushStruct(left);
        flushStruct(right);
        flushStruct(replacementNode);
        sync();
        parent->ptrs[info.nodeIndex] = markDirty(replacementNode);
        flushPtr(&parent->ptrs[info.nodeIndex]);
        sync();
        node->marked = true;
        leafLock.release();
...
int ABTreeOptik<RecordManager, K, V, DEGREE, Compare>::fixTagViolation(const int tid, nodeptr viol) {
    while (true) {
        ...
        if (size <= b) {
            ...
            flushStruct(absorber);
            sync();
            gParent->ptrs[info.parentIndex] = markDirty(absorber);
            flushPtr(&gParent->ptrs[info.parentIndex]);
            sync();
            node->marked = true;
            parent->marked = true;
            RECLAIM_NODE(node);
            ...
            flushStruct(newNode);
            sync();
            gParent->ptrs[info.parentIndex] = markDirty(newNode);
            flushPtr(&gParent->ptrs[info.parentIndex]);
            sync();
            node->marked = true;
            parent->marked = true;
    ...
    while (true) {
        ...
        if (size < 2 * a) {
            ...
            if (gParent == entry && psize == 2) {
                ...
                flushStruct(newNode);
                flushStruct(newParent);
                sync();
                gParent->ptrs[info.parentIndex] = markDirty(newParent);
                flushPtr(&gParent->ptrs[info.parentIndex]);
                sync();
                node->marked = true;
                parent->marked = true;
