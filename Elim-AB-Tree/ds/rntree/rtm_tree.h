...
class Btree : public Index<K, V, size>
{
    ...
    void generateNextLeaf(leaf_node_t *leaf, K &sep)
    {
        ...
        *pointer = leaf;
		...
		int next_split = log->slot[0] - split;
        ...
        next->persist_entry = next->entry = next->slot[0] = next_split;
        next->next = leaf->next;
        leaf->next = next;
    ...
    bool modify(K key, V value, double latency_breaks[3], bool remove = false)
    {
        ...
        while (true)
        {
            ...
            bool res = htmLeafUpdateSlot(leaf, key, entry);
  ...
  public:
    ...
    size_t getSumOfKeysH(node_t* node) {
        if (!node->isLeaf()) {
            inner_node_t* inner_node = static_cast<inner_node_t*>(node);
            size_t total = 0;
            for (int i = 0; i < inner_node->children_number; ++i) {
                total += getSumOfKeysH(inner_node->children_ptrs[i]);
            }
            return total;
        } else {
            leaf_node_t* leaf = static_cast<leaf_node_t*>(node);
            size_t sum = 0;
            for (int i = 1; i < leaf->slot[0] + 1; ++i) {
                // cout << (size_t)(leaf->data[leaf->slot[i]].key) << endl;
                sum += leaf->data[leaf->slot[i]].key;
            }
            return sum;
        }
    }
